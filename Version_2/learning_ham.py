...
def learning_ham (data):
	# We create the table in the database if it doesn't exist and an index to accelerate the research
	cursor.execute("CREATE TABLE IF NOT EXISTS words_ham(id INTEGER PRIMARY KEY, word VARCHAR(256), occurence INTEGER)")
	cursor.execute("CREATE INDEX IF NOT EXISTS words_ham_word_idx ON words_ham (word)")
	db.commit()
	...
	try:
		...
		for new_word in table_word:
			cursor.execute("SELECT * FROM words_ham WHERE word = ?", (new_word,))
			present = cursor.fetchone()
			if present is None:
			    cursor.execute("INSERT INTO words_ham(word, occurence) VALUES(?,?)", (new_word, table_word[new_word]))
			else:
				prev_occur = present[2]
				cursor.execute("UPDATE words_ham SET occurence = ? WHERE word = ? ",(prev_occur + table_word[new_word],new_word))
		...
		present = cursor.fetchone()
		if present is None:
		    cursor.execute("INSERT INTO stat(name, value) VALUES(?,?)", ("nbr_mails_ham", len(files)))
		else:
			value = present[2]
			cursor.execute("UPDATE stat SET value = ? WHERE name = ? ",(value + len(files), "nbr_mails_ham"))
